#!/usr/bin/env python3
"""
Generate all VO scenes using Frank (Wise, Deep and Motivational).
Settings from Jesse's screenshot:
  - Model: eleven_multilingual_v2
  - Speed: ~0.75 (slow, deliberate)
  - Stability: 0.65
  - Similarity: 0.72
  - Style Exaggeration: 0.28
"""
...
VOICE_ID = "V2bPluzT7MuirpucVAKH"  # Frank - Wise, Deep and Motivational
MODEL = "eleven_multilingual_v2"
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "audio-frank")
...
SCENES = [
    ...
    (
        "s06",
        "We hung our lives off wires and roads and pipes. "
        "We handed our water to the reservoir. Our power to the grid. "
        "Our planning to a committee. We lived where the system allowed us to.",
    ),
    (
        "s07",
        "Every home we have ever built has been built by necessity. "
        "We have lived where the river ran and the land was rich and the roads ran through... "
        "never where we longed to.",
    ),
    (
        "s08",
        "The real renegades aren't building cities anymore. "
        "We're walking back out into the land. Taking everything we need with us.",
    ),
...
print(f"Generating {len(SCENES)} VO scenes with Frank...")
...
for scene_id, text in SCENES:
    ...
    if resp.status_code == 200:
        ...
        print(f"         saved {len(resp.content)//1024}KB -> {out_file}")
    else:
        print(f"  [ERROR] {scene_id}: {resp.status_code} {resp.text[:200]}")

    time.sleep(0.5)

done = [s for s, _ in SCENES if os.path.exists(os.path.join(OUT_DIR, f"{s}.mp3"))]
print(f"\nDone: {len(done)}/{len(SCENES)} scenes in {OUT_DIR}")
