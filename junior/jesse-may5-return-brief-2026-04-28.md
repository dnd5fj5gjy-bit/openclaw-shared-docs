# Jesse — May 5 Return Brief
**Prepared:** April 28, 2026
**Author:** Junior
**Purpose:** You're back from filming. You have 48 hours before sailing. This is the full picture.

---

## Your May 5 Schedule

**10:00-11:00 — Coffee: Alice, Jesse, Tom**
Location: Liberty Global offices, 120 King's Road, London SW3 4TR
Organizer: Tom Inskip
Who: Alice Driscoll (ConsultingPartners, +44 7971 019377 / alice@consultingpartners.co.uk) + Tom Inskip

**12:00-12:30 — Tattoo removal**
(personal, no prep needed)

---

## Who Alice Driscoll Is (and What This Meeting Is)

Alice Driscoll has 30+ years of PR and marketing experience and runs ConsultingPartners. She also handles the PR account for Henri-Lloyd, the premium British sailing and outdoor brand.

You've been meeting with Alice and Tom since February 2026:
- Feb 2: First Zoom (Alice organized)
- Feb 27: In-person "Jesse Grylls x Tom x Alice"
- May 5: Third meeting, now "Alice X Jesse X Tom" (Alice listed first — she may have escalated to being the key decision-maker)

The context: you sent Tom an email on April 9 ("A plan") saying "when I wrote to you about how Alfred needs to position itself, I felt like something clicked. I know how to bu[ild this]..." — you're pitching Bear Witness as Alfred's digital media partner, with Tom as the connector.

**Henri-Lloyd is already signed and active.** Not pending — done. Kit was delivered March 2026. Emilia Renshaw (Henri-Lloyd) sent Instagram post briefs in March. On April 23 (the day before you flew to Georgia), you wrote your own ambassador bio for Henri-Lloyd's website: "I'm Jesse Grylls. Adventure has been in the family for as lo[ng as I can remember]..." The ambassador page is being built right now. Alice organized the original intro and is Henri-Lloyd's PR. She'll likely want to discuss how the ambassador content is landing.

**What to expect May 5:** This meeting is probably both Alfred and Henri-Lloyd. Alice manages Henri-Lloyd's PR and was the connector. Tom is the Alfred link. You may get updates on both. If the Henri-Lloyd ambassador page is live by May 5, go prepared to discuss content direction.

**The pitch (Alfred side):** Bear Witness manages Alfred's digital presence — social, content, community narrative. Tom knows Alfred's people. Alice validates the PR credentials. You bring the brand distribution.

---

## The CJ Holding Summary (What Happened While You Were Off)

CJ managed Apr 29 - May 4. Here's what to check when you land:

### Almost certainly done (CJ's priority list)
- Calvin contracts chase (daily) — check if signed or still outstanding
- BPS Hub email to Sam Cotton at leasing@bpsdc.co.uk — CJ was instructed to send this week
- Ships A Lot Slack join (#modern-savage) — straightforward
- Raemy flagged on Modern Savage entity for Stripe

### Needs your eye on return
**Calvin contracts** — if still unsigned, this becomes your first WhatsApp home. Do not let this drift another week. You sail May 7.

**Modern Savage Stripe + US entity** — The entity question is more complex than a simple UK Ltd. On April 20, you, CJ, Bear, and Raemy were discussing a US C-corp structure (Bear Witness wholly-owned subsidiary), with funding via BGV LLP's Citizens Bank account. This was not resolved. Also: THREE ranges are launching on the same SuperHero platform (Modern Savage, BG Summit, and a third "Min" range). Raemy needs to confirm entity for all three before any Stripe setup. If this wasn't resolved on Tuesday April 28, it needs to happen before May 21 launch prep resumes.

**Rick Landman callback** — Your April invoice at Cavendish Trust is held pending a 30-second call from you to verify bank account details. Rick's mobile: **+447624277888**. Do this before the Alice/Tom coffee. It takes literally 30 seconds and releases your April pay.

**Email 1 to Modern Savage waitlist** — 170 people. Calvin needs Stripe live first, then sends. Check status.

---

## What's Moving Without You

**Wildernests BCG engagement (Gunnar Trommer + Alex Aboshiha):**
CORRECTED: this is a WILDERNESTS engagement, not Ted's Health. Gunnar Trommer leads BCG X's North America Healthcare practice (based Hermosa Beach, CA). Alex Aboshiha is the Wildernests CEO. They've been running weekly calls with Jesse since at least April 9. Gunnar sent Revit architectural renderings April 6. A fourth person "(Joe)" joined from April 22. **The May 7 weekly call is at 5pm — your sailing day.** Decide before then: dial in from the boat, ask them to reschedule for May 5 or 6, or skip and reconnect when back. Don't just miss it without a heads-up. Alex left a note April 16 asking Gunnar for "a fresh external proposal on what you think would be the very best solution" — a BCG strategy proposal for Wildernests may be waiting when you return May 21.

**EBG / Benjamin Simon — Modern Savage Investment Pitch:**
More significant than it first appeared. Benjamin Simon (CEO, Everest Brand Group) emailed you April 14 with Bear and Will Gardenswartz (EBG) cc'd — meaning Bear was already in the loop before Benjamin contacted you. Benjamin's words: "there may be an opportunity for us to work together — including a potential path for the Fund to invest and help accelerate growth." The Fund = Sovereign's Capital Everest Fund I — EBG's investment vehicle. This isn't a content partnership conversation — it's an investment offer.

Zoom happened April 16 at 9:30 EST. No follow-up email. You know what was discussed. Before sailing, send Benjamin a holding note either way: ben@everestbrandgroup.com. Will Gardenswartz is the EBG contact cc'd: will@everestbrandgroup.com.

**Fat Cow / Ryan Porter:**
Ryan Porter (Director, Fat Cow) submitted a pitch deck April 15. You reviewed it and replied April 24: "I want to keep our first move simple." You organized a follow-up Zoom with Ryan for April 27 noon — but you were on set in Georgia. That Zoom almost certainly didn't happen. Ryan has been waiting since April 27. Send him a quick message before sailing: "Ryan — apologies, was on set. Let's reconnect when I'm back May 5/6."

**Running Wild S9:** CJ is approving content from May 7 onwards. All episode briefs are built and in the CJ handover. MGK episode (May 5) needs Jesse approval — Calvin should have sent you draft posts by May 3. Check your email for Metricool review requests from Calvin.

**Water2:** If the Charles Robinson call didn't happen before April 28, CJ should have sent a holding email. Check email from CJ for update.

**Wildernests / Tattarang:** If Taki Dermedgoglou (Tattarang/Twiggy Forrest family office) hasn't responded to Bear's April 23 intro, CJ should have followed up. This is the $20B investor intro — don't let it go cold.

---

## What Alfred Actually Needs

Based on what I've found, Alfred is a premium members' network with a strong social/in-person community. Tom Inskip has been circling a commercial relationship where Bear Witness provides the digital media layer — social strategy, content, brand narrative.

**Your edge:** You've helped Bear build social infrastructure across multiple brands. Alfred is trying to grow a community of the type of people who follow Bear. The pitch isn't "we make posts" — it's "we understand how your members think and we know how to build the digital community that makes Alfred the thing they talk about."

**What Alice brings:** PR and marketing validation. She's not the decision-maker for Alfred's operations — she's likely either advising Alfred, advising Tom, or evaluating whether Bear Witness is the right fit before Tom introduces you to Alfred's founder/CEO.

**Who you haven't met yet:** Alfred's actual founder/leadership. May 5 may be Alice and Tom aligning before they make that introduction. Be patient — this is a relationship sale.

---

## The May 5-7 Window: What Must Happen Before You Sail

In priority order:

1. **Rick Landman callback** — 30 seconds, +447624277888, release your April pay
2. **Calvin contracts check** — if still unsigned, WhatsApp Calvin: "Contracts. Today. I sail Thursday."
3. **Modern Savage Stripe** — confirm with Raemy entity is set up; if not, action it now
4. **CJ debrief** — 15-minute call: what happened while you were off, what needs Jesse before sailing
5. **Alice/Tom coffee** — briefed above, go in knowing the Henri-Lloyd kit deal status
6. **MGK Running Wild posts** — approve before sailing; CJ handles from May 7

**Things that can wait:** Water2 follow-up (email from CJ holding warm), BPS Hub (CJ sent the email), Alfred formal pitch (May 5 is prep, not close), Wildernests equity docs (CJ monitoring).

---

## Key Contacts for Quick Reference

| Person | Role | Contact |
|--------|------|---------|
| Rick Landman | Cavendish Trust — invoice callback | +447624277888 |
| Alice Driscoll | ConsultingPartners — May 5 coffee | alice@consultingpartners.co.uk |
| Tom Inskip | Alfred connector | Via calendar invite |
| Raemy Singh | CFO — Modern Savage entity | Raemy@beargryllsventures.com |
| CJ Cardenas | MD BGV — handover debrief | Via BGV |

---

## Things I Couldn't Fully Resolve (Flag to Jesse)

**Henri-Lloyd deal status:** The February Heads of Agreement said you were "all good with the terms" — is this signed? Emilia Renshaw is the Henri-Lloyd contact (emilia from Henri-Lloyd). With Alice at the May 5 meeting, this may come up. Worth knowing where it stands.

**Alfred founder/CEO identity:** I couldn't find who actually runs Alfred or what stage the company is at. Tom knows. Ask him at the coffee.

**Wildernests Tattarang follow-up:** I couldn't confirm if CJ reached out to Taki Dermedgoglou. Check with CJ.

---

*This brief was built from your calendar, email archive, and business context files. It covers everything I could see. What I can't see is what happened in the room at the April 20 Alfred meeting — that's still in your head.*
