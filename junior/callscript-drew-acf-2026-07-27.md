# Monday's call to Drew. Read straight off the page.

**Andrew Kouvel, ACF Pharma. 631 484 7042 (cell, WhatsApp, Telegram). US Eastern, so from 13:00 BST.**

Verified at source 26 Jul: number and channels are off his own email signature, and ACF Pharma is at 610 SW 12th Ave, Pompano Beach, Florida, so Eastern is right. He signs email as **Drew**, and your phone has him saved as **Andy Kouvel**. Same man.

**Use WhatsApp, not a cold dial.** It is the only channel with a proven result: you and he had a WhatsApp call on Tue 22 July at 14:14, and you messaged him at 14:22 straight after. Four emails have failed and one WhatsApp call worked. Open the thread you already have rather than ringing a number cold.

You are not chasing artwork. You are getting one of two things: the print-ready back panel, or a date he will say out loud. Either is a result. A friendly non-answer is not.

Do not open by referencing the failed emails. Open warm and assume nothing is wrong.

---

## The opener, verbatim

> "Drew, it's Jesse. Sorry to call on a Monday, I'll be quick. We're at the point where the pouch artwork is the only thing standing between us and the print run, and I'd rather sort it in two minutes on the phone than another email chain. Where are you with the back panel?"

Then stop talking. Let the silence do the work.

**Good answer sounds like:** a specific state, "it's with my formulator", "I need the final fill weight", "I can send it Wednesday".
**Red flag sounds like:** "yeah it's coming", "let me check and come back to you", or any answer that does not contain a noun or a date.

---

## Question 1, if he gives you anything vague

> "Totally understood. What do you need from us to finish it? I've got Em and Ku on standby today, so if it's something on our side I can clear it this afternoon."

**Good:** he names a specific missing input. Write it down word for word and repeat it back.
**Red flag:** he says he needs nothing and it's just time. That means it has not been started.

## Question 2, the one that actually matters

> "What date can you commit to for the print-ready panel? I'm not after a best case, I want the date you'd be comfortable holding me to."

**Good:** any date on or before Wednesday 29 July.
**Red flag:** "end of the week", "next week", or a date without a day of the week attached.

If he offers a date after Wednesday 29 July, say this:

> "I have to be straight with you about why that's a problem. Bear is on stage at the Health Optimisation Summit on 12 September and that date cannot move. Working back through Belmark's print run, the fill and the freight, anything past Wednesday means we don't have product to sell at the single biggest moment this brand gets this year. So if Wednesday genuinely isn't possible, tell me now and I'll plan around it, but I need to know today."

## Question 3 — CHANGED 26 JUL. Do not ask him for the address.

The earlier version of this script had you asking Drew for the samples shipping address. **You already gave it to him.** On WhatsApp, Tue 22 July at 14:22, eight minutes after your call:

> Jesse Grylls, Abersoch Boatyard Services, The Saltings, Pwllheli, LL53 7AR, United Kingdom

Sarah asked him for that address on 22 July and again on 23 July, and he has not answered her either time. So the address exists, it has been sitting with him for four days, and Belmark are still waiting on it. Asking him for it again would have you chasing your own information.

Say this instead:

> "One tiny thing while I've got you. I sent you the samples address on WhatsApp last Tuesday, the boatyard in Pwllheli. Sarah's asked twice and she's still waiting, so did that reach her? If it's easier I'll just send it to her myself now."

**Good:** he says he'll forward it, or says send it yourself.
**Red flag:** he does not remember it. That tells you he has not been reading either thread, which is your real answer about everything else on this call.

**Worth knowing before you decide:** that is a Welsh boatyard, not the copacker. On 16 July you told Sarah to send the blanks to the copacker for a trial fill. If you have since decided to see them yourself first, fine, but Belmark should be told which it is, because a trial fill and a look are different jobs. Tell me which and I will put it to Sarah in one line.

## Question 4, only if the mood is good

> "And the spec sheets, are we able to get those signed off this week? They're holding up the UK manufacturers as well, so it unlocks two things at once."

---

## The close, verbatim

> "That's really helpful. So just so I've got it straight: you're sending me [X] by [day], and the shipping address is [Y]. I'll email you a two-line summary of that when we hang up so we both have it in writing. Thanks Drew."

**Send that email within five minutes of hanging up.** Not as a formality, as the record. A date agreed on a call and never written down is not a date.

---

## After the call, text me one line

Just one of these and I'll take it from there:

- **"Panel [day]"** and I chase Belmark to hold the slot
- **"No date"** and I have the pre-order contingency ready to go for Tuesday
- **"No answer"** and we make Tuesday's decision without him

---

**If he does not pick up:** WhatsApp him the same opener as a voice note rather than typing it. He lists WhatsApp on his own signature, and a voice note from you is much harder to leave unanswered than a fifth email. If nothing by Monday evening, that is itself the answer and Tuesday's decision gets made without him.
